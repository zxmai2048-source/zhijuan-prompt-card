...
- en.prompt is the default UI generation prompt. Keep it one line, neutral, as long as needed for faithful reconstruction, with no fixed word cap. Order when useful: fingerprint, skeleton, anchors, composition, light, materials/text, fidelity locks. Dense multi-region, text-heavy, multi-subject, or layout-critical images should expand as needed. Do not squeeze load-bearing structure into a generic archetype. json_prompt.generation_prompt is the advanced high-fidelity generator input and must not be weaker than en.prompt.
- prompt_core is a compact English summary of subject, composition, lighting, palette, and source style.
- negative_prompt is image-specific, normally 8-24 items. Block drift: wrong identity/source, count, face/body/skin tone, pose, surface/material, moved/translated text, redesigned UI/layout, wrong crop/viewpoint, missing props, extra objects, over-polish, artifacts. If source-defining geometry is curved, diagonal, irregular, organic, or continuous, block straightened bands, rectangular panels, uniform grids, equalized zones, and lost boundary lines. Do not stack generic blockers.
